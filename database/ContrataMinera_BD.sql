...
create view DSelectorReparto as
select codReparto, fechaReparto, concat(apePaterno, ' ', apeMaterno, ' ' , nombres) as Trabajador, nombreGuardia, nombreTurno, idVehiculo from reparto r
inner join trabajador t on t.idTrabajador = r.idTrabajador
inner join guardia g on g.codGuardia = r.codGuardia
...
begin 
	...
	insert into vale (codVale, fecha, hora, lugar, horometro, galones, codReparto)
    values  (p_codVale, p_fecha, p_hora, p_lugar, p_horometro, p_galones, p_codReparto);
    commit;
end$$
delimiter ;
